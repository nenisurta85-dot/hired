...
function AdminTasks() {
  ...
  const groupedTasks = React.useMemo(() => {
    ...
    const tasks = flatTasks().filter((t) => {
      ...
      if (!isTaskInPeriod(t)) return false;
      if (!showCompleted && t.status === "complete") return false;
      return true;
    });
    const groups = [
      { key: "overdue",     label: "Overdue",     headerBg: "#FDEAEA", headerFg: "#C0241F", tasks: tasks.filter(t => t.status === "overdue") },
      { key: "in_progress", label: "In Progress",  headerBg: "#E6F1FB", headerFg: "#185FA5", tasks: tasks.filter(t => t.status === "in_progress") },
      { key: "not_started", label: "Not Started",  headerBg: "#F1EFE8", headerFg: "#5F5E5A", tasks: tasks.filter(t => t.status === "not_started") },
    ];
    if (showCompleted) groups.push({ key: "complete", label: "Complete", headerBg: "#E8F5EE", headerFg: "#0F9E75", tasks: tasks.filter(t => t.status === "complete") });
    return groups.filter(g => g.tasks.length > 0);
  }, [selClient, selProject, selPhase, selTaskType, period, showCompleted]);
  ...
  const TaskCard = ({ t }) => {
    ...
    return (
      <div className="task-card clickable" style={{ borderLeft: `3px solid ${border}` }}
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
        {["Today", "This Week", "This Month"].map(p => (
          <button key={p} onClick={() => setPeriod(p)}
            style={{ fontSize: 12, fontWeight: 600, padding: "5px 14px", borderRadius: 99, border: "1.5px solid", cursor: "pointer", transition: "all .12s",
        ...
        : groupedTasks.map(g => (
          <div key={g.key} style={{ marginBottom: 24 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 10px", borderRadius: 99, background: g.headerBg, color: g.headerFg }}>{g.label}</span>
              <span style={{ fontSize: 11, color: "var(--text-muted)", fontWeight: 600 }}>· {g.tasks.length}</span>
            </div>
